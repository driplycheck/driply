...
const UPDATES = [
  {
    ...
    emoji: ['check', 'arrow'],
    text: `если что-то сломалось — напиши ➡️

в driply появился раздел «поддержка»: настройки → поддержка → пишешь, что случилось

читаю лично, отвечаю сообщением от бота

баг, идея, вопрос по дрипам, что-то не грузится — всё сюда. чем конкретнее, тем быстрее починю ✅`,
    why: 'Раньше «Поддержка» просто открывала бота, а бот молчал на обычные сообщения. Пост нужен, чтобы люди знали: канал обратной связи наконец работает.',
  },
  {
...
плюс 200 дрипов за саму историю, раз в сутки`,
  ...
  },
  {
    ...
    text: `у первых ста появился статус 💧

выложил образ и попал в первую сотню авторов — в профиле навсегда остаётся FIRST DRIP
он же уходит в карточку, которой делишься в сторис

сотня не резиновая, дальше статус не выдаётся`,
    why: 'Дефицит + статус навсегда. Работает как повод выложить образ сегодня, а не потом.',
  },
  {
